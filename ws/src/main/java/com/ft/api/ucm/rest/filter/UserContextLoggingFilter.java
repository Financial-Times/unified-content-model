...
public class UserContextLoggingFilter extends Log4jNestedDiagnosticContextFilter {
	...
	private void appendCurrentLoggingContext(HttpServletRequest request, StringBuilder message) {
        ...
        if(!StringUtils.isBlank(apikey)) {
			message.append(API_KEY_LABEL)
                   .append('=')
                   .append(apikey)
                   .append(' ');
		}
		
		message.append(USER_IP_LABEL)
               .append('=')
               .append(StringUtils.isBlank(ip) ? request.getRemoteAddr() : ip)
               .append(' ');

        message.append(this.CONTENT_CONTROL_POLCIY_LABEL)
               .append('=')
