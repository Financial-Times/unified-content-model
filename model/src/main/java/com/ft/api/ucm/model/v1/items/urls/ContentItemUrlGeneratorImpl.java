...
public class ContentItemUrlGeneratorImpl implements ContentItemUrlGenerator {
  ...
  static final String CONTEXT_REGEX =
      "http[s]*://[A-Za-z0-9-.]+/content/[content|structure]([A-Za-z0-9-/.?=]+)";
  ...
  @Override
  public String createUrlForItemWithLastModifiedDate(
  ...
  private String alignProtocol(String generateUrl, HttpProtocol httpProtocol) {
    switch (httpProtocol) {
      case HTTPS:
        return generateUrl.replace(HTTP, httpProtocol.getValue());
      default:
        return generateUrl;
    }
  }
